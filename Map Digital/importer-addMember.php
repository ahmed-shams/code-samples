								...
								if (!isset($_SESSION['seleventid'])) die("Action cannot be taken on the selected event please close the window and re-login.");
									...
									if ($k == $arcolumnno) {
										...
										if ($k == $iiscolumnno) {
											...
											if (!in_array(strtolower($rows[$j][$k]), $optionsarr)) {
//												if (!array_key_exists(str_replace(' ', '', strtolower($rows[$j][$k])), $nospacesoptions)) {
												if (!array_key_exists(str_replace(' ', '', ($rows[$j][$k])), $nospacesoptions)) {
...
// 											}
							...
							//TODO:
							// Add check to see if email already exists and update instead of insert
							
							$linemysql = implode("','", $rows[$j]);
							if ($addauto) {
								$query = "insert into $databasetable values('','$eventid','Pre-Reg','$linemysql');";
							} else {
								$query = "insert into $databasetable values('$eventid','Pre-Reg','$linemysql');";
							}
							$queries.= $query . "\n";
			...
			case 'add':				
				if (isset($_POST['Fieldname']) && isset($_POST['Fieldtype']))
				{
					...
					else
					{
						...
						if ((strcasecmp($_POST['Fieldtype'], TYPE3) === 0) || (strcasecmp($_POST['Fieldtype'], TYPE8) === 0))
						{
							...
							else
							{
								...
								if (!fieldExists($fieldname)) {
									...
									$fnmessage = "Fieldname '$fieldname' already exists for this event. Please choose another Fieldname";
								}
							}
						}
						else
						...
						if ($success) {
							...
							$fnmessage = (empty($fnmessage)) ? "Unsuccessful action: Fieldname already exists/Event is not selected to edit/Options are not added successfully. Please try again." : $fnmessage;
						}
...
<fieldset style='width:90%'>
	<legend><b>Upload Attendee List</b></legend>
	<div id='uploaddiv' style='margin-left:30'>
<?php 
...
$upl_addform->addFormPart('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick=window.open("downloadTemplate.php?action=UploadForm&todo=UploadRequirements&eventselected=' . $tempseleventid . '","","toolbars=no,menubar=no,location=no,scrollbars=yes,resizable=yes,status=yes,width=900,height=700")>Requirements to fill the template</a>');
$upl_addform->addFormPart('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick=window.open("importer/index.php?eventselected=' . $tempseleventid . '","","toolbars=no,menubar=no,location=no,scrollbars=yes,resizable=yes,status=yes,width=600,height=500")>Import User List</a><br>');
$upl_addform->addFormPart();
$eveuperror = $esuccess = '';
$error = false;
