<?PHP
	...
	*	SCRIPT TYPE: Class  																						*											
	*	SCRIPT NAME: classDB																						*	
	*	DESCRIPTION: General Class to access database functions														*		
	*	WRITTEN ON: 25-Dec-2004																						*	
	*	LAST MODIFIED ON:																							*	
	...
	*****************************************************************************************************************/
	class classDB {
		...
		$objDB = new classDB();
		$objDB->dbConnection($Server, $Database, $Username, $Password);
		...
		*/		
		...
		function dbConnect() {
			...
			if ($this->dbConnected==0) {
				$this->dbLink = mysql_connect($this->dbServer, $this->dbUser, $this->dbPass) or $this->reportError("Could not connect to Server.");
				//doPrint("CONNECTING DATABASE : LINK => ".$this->dbLink);
				$_REQUEST["DB_CONNECTED"] 	= $this->dbConnected = 1;
				$_REQUEST["DB_LINK"] 		= $this->dbLink;		
				mysql_select_db ($this->dbDatabase, $this->dbLink) or $this->reportError("Could not select Database.");	
			} 
		}
			...
			if ($this->dbConnected == 1) {
				if ($this->dbQuerySet == 1) {				
					...
					case 1:
						...
						//}
						$this->dbQueryResult = mysql_query ($this->dbQuery, $this->dbLink) or $this->reportError();
					...
					if ($this->dbQueryResult<>"") {
						...
						if ($this->dbQueryTypeSet == 1) {
							if ($this->dbQueryType == "select") {
								switch ($this->dbType) {
								case 1:
									$this->dbQueryNumRows = mysql_num_rows($this->dbQueryResult) or $this->dbQueryNumRows = 0;
									break;	
								case 2:
								...
								switch ($this->dbType) {
								case 1:
									$this->dbQueryNumRows = mysql_affected_rows($this->dbLink) or $this->dbQueryNumRows = 0;
									$this->dbLastInsertRow = mysql_insert_id($this->dbLink);
									break;	
								case 2:
		...
		function dbFreeResultSet() {
			if ($this->dbResultSetInMem == 1) {
				switch ($this->dbType) {
				case 1:
					mysql_free_result($this->dbQueryResult);
					break;	
				case 2:
		...
		function dbSelectQuery() {
			...
			if ($this->dbQueryNumRows > 0) {
			   ...
			   switch ($this->dbType) {
				case 1:
					while ($row = mysql_fetch_array($this->dbQueryResult)) {
						$record[] = $row; 				
					}	
					break;	
				case 2:
		...
		function reportError($error = "") { // Sends Error Report 1. To logfile 2. To Email 3. system logs
			...
			print mysql_error()."<br>CHECK THE URL => <br>".$this->dbQuery;
			switch ($global_config["error_report"]) {
				case 1:	// reportError in logfile			
						echo $this->dbQuery."<br>".mysql_error();
			...
			if($global_config["debug_mode"]) {
				switch ($this->dbType) {
				case 1:
					$error_string = "<b>Error: </b><i>".$error."</i><br><b>Reason: </b>".mysql_error();		
					break;	
				case 2:
		...
		function mysqliDBConnect(){
			$this->objDBLayer = new DBLayer($this->dbServer,$this->dbUser,$this->dbPass,$this->dbDatabase);
		}
		function mysqliDBExecuteQuery($strSQL,$queryType){
			$this->mysqliDBConnect();
			$rsRecord = array();
			if($queryType=="multi_query"){
				$rsRecord = $this->objDBLayer->multi_query($strSQL);
			}else{
				$queryResult = $this->objDBLayer->query($strSQL);
				if($queryType=="fetch_assoc")
					$rsRecord = $this->objDBLayer->fetch_assoc($queryResult);
				if($queryType=="fetch_row")
					$rsRecord = $this->objDBLayer->result($queryResult);
				if($queryType=="fetch_array")
					$rsRecord = $this->objDBLayer->fetch_array($queryResult);
				if($queryType=="num_rows")
					$rsRecord = $this->objDBLayer->num_rows($queryResult);
				if($queryType=="test")
					$rsRecord = $this->objDBLayer->test($queryResult);
				$this->objDBLayer->free_result($queryResult);
			}
			$this->objDBLayer->close();
			return $rsRecord;
		}	
	}	
?>