...
function InnovaEditor(oName)
    ...
    this.dropTopAdjustment_moz=-1;//0;
	...
	this.buttonMap=["Preview","FullScreen","Print","Search","SpellCheck","|",
		  "Cut","Copy","Paste","PasteWord","PasteText","|","Undo","Redo","|",
		  "ForeColor","BackColor","|","Bookmark","Hyperlink",
		  "Image","Flash","Media","ContentBlock","InternalLink","InternalImage","|","Table","Guidelines","Absolute","|","BRK","CustomObject",
		  "Characters","Line",
		  "Form","RemoveFormat","HTMLFullSource","HTMLSource","XHTMLFullSource",
		  "XHTMLSource","ClearAll",
		  "StyleAndFormatting","Styles","|","CustomTag","Paragraph","FontName","FontSize","BRK","|",
    ...
    this.useTagSelector=false; //true;
    ...
    for(var i=0;i<this.buttonMap.length;i++)
        ...
        switch(sButtonName)
            ...
            case "Preview": 
                ...
                if(this.btnPreview)
                    { 
					  if (this.btnPreviewFlag) {
						imgpreview="btnPreview1.gif";
					  } else {
						imgpreview="btnPreview.gif";
					  }
              	      sHTMLIcons+=this.writeIconStandard("btnPreview"+this.oName,this.oName+".dropShow(this,'dropPreview"+this.oName+"')",imgpreview,getTxt("Preview"));

					  /*if (this.btnPreviewFlag) {
					  sHTMLIcons+=this.writeIconStandard("btnPreview"+this.oName,this.oName+".dropShow(this,dropPreview"+this.oName+")","btnPreview1.gif",getTxt("Preview"));
					  } else {
                    ...
                    { 
					 if (this.btnPreviewFlag1==false) {
                        ...
                        sTmp+= "<tr><td onclick=\"document.getElementById('dropPreview"+this.oName+"').style.display='none';setActiveEditor("+this.oName+");window.open('"+this.scriptPath+"preview.php','previewWindow1','width="+arrPreviewSize[j][0]+",height="+arrPreviewSize[j][1]+",location=no,directories=no,status=no,menubar=no,resizable=yes,scrollbars=yes,top=0,left=0');\" "+
                        // sTmp+= "<tr><td onclick=\"alert('in');document.getElementById('dropPreview"+this.oName+"').style.display='none';setActiveEditor("+this.oName+");modalDialogShow('"+this.scriptPath+"preview.php',"+arrPreviewSize[j][0]+","+arrPreviewSize[j][1]+");\" "+
						    "style=\"padding:2px;padding-top:1px;font-family:Tahoma;font-size:11px;color:black;\" "+
                            "onmouseover=\"this.style.backgroundColor='#708090';this.style.color='#FFFFFF';\" "+
                            "onmouseout=\"this.style.backgroundColor='';this.style.color='#000000';\" unselectable=on>"+arrPreviewSize[j][0]+"x"+arrPreviewSize[j][1]+"</td></tr>";
					 }
